...
public class MessagePacker implements Closeable, Flushable {
   ...
   private void flushBuffer() throws IOException {
      ...
      this.buffer = null;
      this.totalFlushBytes = this.totalFlushBytes + this.position;
      this.position = 0;
   }
   ...
   public MessagePacker packLong(long v) throws IOException {
      if (v < -32L) {
         if (v < -32768L) {
            ...
            this.writeByteAndShort((byte)-47, (short)v);
         } else {
            this.writeByteAndByte((byte)-48, (byte)v);
         }
      } else if (v < 128L) {
         ...
         if (v < 256L) {
            ...
            this.writeByteAndShort((byte)-51, (short)v);
         }
