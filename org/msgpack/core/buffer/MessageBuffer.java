...
public class MessageBuffer {
   ...
   public MessageBuffer slice(int offset, int length) {
      if (offset == 0 && length == this.size()) {
         ...
         return new MessageBuffer(this.base, this.address + offset, length);
      }
   }

   public byte getByte(int index) {
      return unsafe.getByte(this.base, this.address + index);
   }

   public boolean getBoolean(int index) {
      return unsafe.getBoolean(this.base, this.address + index);
   }

   public short getShort(int index) {
      short v = unsafe.getShort(this.base, this.address + index);
      return Short.reverseBytes(v);
   }

   public int getInt(int index) {
      int i = unsafe.getInt(this.base, this.address + index);
   ...
   public long getLong(int index) {
      long l = unsafe.getLong(this.base, this.address + index);
   ...
   public void getBytes(int index, byte[] dst, int dstOffset, int length) {
      unsafe.copyMemory(this.base, this.address + index, dst, ARRAY_BYTE_BASE_OFFSET + dstOffset, length);
   }
   ...
   public void putByte(int index, byte v) {
      unsafe.putByte(this.base, this.address + index, v);
   }

   public void putBoolean(int index, boolean v) {
      unsafe.putBoolean(this.base, this.address + index, v);
   }

   public void putShort(int index, short v) {
      v = Short.reverseBytes(v);
      unsafe.putShort(this.base, this.address + index, v);
   }

   public void putInt(int index, int v) {
      v = Integer.reverseBytes(v);
      unsafe.putInt(this.base, this.address + index, v);
   }
   ...
   public void putLong(int index, long l) {
      ...
      unsafe.putLong(this.base, this.address + index, l);
   }
   ...
   public void putBytes(int index, byte[] src, int srcOffset, int length) {
      unsafe.copyMemory(src, ARRAY_BYTE_BASE_OFFSET + srcOffset, this.base, this.address + index, length);
   }
   ...
   public void putByteBuffer(int index, ByteBuffer src, int len) {
      ...
      if (src.hasArray()) {
         ...
         unsafe.copyMemory(srcArray, ARRAY_BYTE_BASE_OFFSET + src.position(), this.base, this.address + index, len);
         src.position(src.position() + len);
      } else if (this.hasArray()) {
         src.get((byte[])this.base, index, len);
      } else {
         for (int i = 0; i < len; i++) {
            unsafe.putByte(this.base, this.address + index, src.get());
         }
      }
   }

   public void putMessageBuffer(int index, MessageBuffer src, int srcOffset, int len) {
      unsafe.copyMemory(src.base, src.address + srcOffset, this.base, this.address + index, len);
   }

   public ByteBuffer sliceAsByteBuffer(int index, int length) {
      if (this.hasArray()) {
         return ByteBuffer.wrap((byte[])this.base, (int)(this.address - ARRAY_BYTE_BASE_OFFSET + index), length);
      } else {
         assert !isUniversalBuffer;
   ...
   public byte[] toByteArray() {
      ...
      unsafe.copyMemory(this.base, this.address, b, ARRAY_BYTE_BASE_OFFSET, this.size());
      return b;
   }
   ...
   public void copyTo(int index, MessageBuffer dst, int offset, int length) {
      unsafe.copyMemory(this.base, this.address + index, dst.base, dst.address + offset, length);
   }
