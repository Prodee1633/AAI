...
public class MessageBufferBE extends MessageBuffer {
   ...
   public MessageBufferBE slice(int offset, int length) {
      if (offset == 0 && length == this.size()) {
         ...
         return new MessageBufferBE(this.base, this.address + offset, length);
      }
   }

   @Override
   public short getShort(int index) {
      return unsafe.getShort(this.base, this.address + index);
   }

   @Override
   public int getInt(int index) {
      return unsafe.getInt(this.base, this.address + index);
   }

   @Override
   public long getLong(int index) {
      return unsafe.getLong(this.base, this.address + index);
   }

   @Override
   public float getFloat(int index) {
      return unsafe.getFloat(this.base, this.address + index);
   }

   @Override
   public double getDouble(int index) {
      return unsafe.getDouble(this.base, this.address + index);
   }
   ...
   public void putLong(int index, long v) {
      unsafe.putLong(this.base, this.address + index, v);
   }

   @Override
   public void putDouble(int index, double v) {
      unsafe.putDouble(this.base, this.address + index, v);
   }
}
