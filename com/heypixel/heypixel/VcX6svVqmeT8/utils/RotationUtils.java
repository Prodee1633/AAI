...
public class RotationUtils {
   ...
   public static double getMinDistance(Entity target, Vector2f rotations) {
      ...
      while (var4.hasNext()) {
         double eye = ((Float)var4.next()).floatValue();
   ...
   public static HitResult getIntercept(AABB targetBox, Vector2f rotations, Vec3 eyePos, double reach) {
      ...
      return ProjectileUtil.getEntityHitResult(mc.player, eyePos, vec32, targetBox, p_172770_ -> !p_172770_.isSpectator() && p_172770_.isPickable(), reach * reach);
   }
   ...
   private static HitResult rayTrace(Vector2f rotations) {
      double d0 = mc.gameMode.getPickRange();
      ...
      EntityHitResult entityhitresult = ProjectileUtil.getEntityHitResult(mc.player, vec3, vec32, aabb, p_172770_ -> !p_172770_.isSpectator() && p_172770_.isPickable(), d1);
   ...
   public static RotationUtils.Data getRotationDataToEntity(Entity target) {
      Vec3 playerPosition = new Vec3(mc.player.getX(), mc.player.getY(), mc.player.getZ());
      Vec3 eyePos = playerPosition.add(0.0, mc.player.getEyeHeight(), 0.0);
      AABB targetBox = getTargetBoundingBox(target);
      ...
      for (Vec3 point : points) {
         Vector2f bruteRotations = getRotations(eyePos, point);
         ...
         if (checkHitResult(eyePos, bruteHitResult, target)) {
            ...
            return new RotationUtils.Data(
               ...
               getFixedRotation(bruteRotations.x, bruteRotations.y, RotationManager.lastRotations.x, RotationManager.lastRotations.y)
            );
         }
      }
