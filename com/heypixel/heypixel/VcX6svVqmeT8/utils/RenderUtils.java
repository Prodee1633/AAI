...
public class RenderUtils {
   ...
   public static int getRainbowOpaque(int index, float saturation, float brightness, float speed) {
      float hue = (float)((System.currentTimeMillis() + index) % (int)speed) / speed;
   ...
   private static void innerFill(Matrix4f pMatrix, float pMinX, float pMinY, float pMaxX, float pMaxY, int pColor) {
      ...
      float f3 = (pColor >> 24 & 0xFF) / 255.0F;
      float f = (pColor >> 16 & 0xFF) / 255.0F;
      float f1 = (pColor >> 8 & 0xFF) / 255.0F;
      float f2 = (pColor & 0xFF) / 255.0F;
      BufferBuilder bufferbuilder = Tesselator.getInstance().getBuilder();
   ...
   public static void drawRoundedRect(PoseStack poseStack, float x, float y, float width, float height, float edgeRadius, int color) {
      ...
      for (int i = 0; i <= vertices; i++) {
         double angleRadians = (Math.PI * 2) * (i + 180) / (vertices * 4);
         color(buffer, matrix, (float)(centerX + Math.sin(angleRadians) * edgeRadius), (float)(centerY + Math.cos(angleRadians) * edgeRadius), color);
      }
      ...
      for (int i = 0; i <= vertices; i++) {
         double angleRadians = (Math.PI * 2) * (i + 90) / (vertices * 4);
         color(buffer, matrix, (float)(centerX + Math.sin(angleRadians) * edgeRadius), (float)(centerY + Math.cos(angleRadians) * edgeRadius), color);
      }
      ...
      for (int i = 0; i <= vertices; i++) {
         double angleRadians = (Math.PI * 2) * (i + 270) / (vertices * 4);
         color(buffer, matrix, (float)(centerX + Math.sin(angleRadians) * edgeRadius), (float)(centerY + Math.cos(angleRadians) * edgeRadius), color);
      }
      ...
      for (int i = 0; i <= vertices; i++) {
         double angleRadians = (Math.PI * 2) * i / (vertices * 4);
         color(buffer, matrix, (float)(centerX + Math.sin(angleRadians) * edgeRadius), (float)(centerY + Math.cos(angleRadians) * edgeRadius), color);
      }
