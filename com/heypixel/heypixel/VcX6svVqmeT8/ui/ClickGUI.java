package com.heypixel.heypixel.VcX6svVqmeT8.ui;
...
import com.heypixel.heypixel.VcX6svVqmeT8.values.Value;
import com.heypixel.heypixel.VcX6svVqmeT8.values.ValueType;
...
public class ClickGUI extends Screen {
   ...
   public boolean mouseScrolled(double pMouseX, double pMouseY, double pDelta) {
      if (this.bindingModule == null && RenderUtils.isHoveringBound((int)pMouseX, (int)pMouseY, windowX + 5.0F, windowY + 20.0F, 100.0F, windowHeight - 5.0F)) {
         this.categoryMotionY.target = (float)(this.categoryMotionY.target + pDelta * 15.0);
      ...
      if (this.renderValues != null
         ...
         this.moduleValuesMotionY.target = (float)(this.moduleValuesMotionY.target + pDelta * 15.0);
   ...
   public void render(GuiGraphics g, int mouseX, int mouseY, float pPartialTick) {
      ...
      for (Category value : Category.values()) {
         ...
         float height = value.ordinal() * 25 * (yAnimation.value / 255.0F);
      ...
      opensans.render(stack, "Naven", windowX + 50.0F - opensans.getWidth("Naven", 0.75) / 2.0F, windowY + 5.0F, Color.WHITE, true, 0.75);
      opensans.setAlpha(1.0F);
      if (this.selectedCategory != null) {
         ...
         this.hoveringBack = RenderUtils.isHovering(
            ...
            (float)(windowY + 5.0F + opensans.getHeight(true, 0.4F))
         );
         if (this.hoveringBack && !this.clickResizeWindow && !this.clickDragWindow) {
      ...
      if (this.categoryModules != null) {
         ...
         for (Module categoryModule : this.categoryModules) {
            ...
            opensans.setAlpha(alpha / 255.0F);
            opensans.render(
               stack, categoryModule.getName(), windowX + 13.0F, windowY + 25.0F + renderModuleHeight + this.categoryMotionY.value, Color.WHITE, true, 0.4
            );
            opensans.setAlpha(1.0F);
      ...
      if (this.renderValues != null) {
         ...
         for (Value value : this.renderValues) {
            if (value.isVisible() && value.getValueType() == ValueType.BOOLEAN) {
               ...
               font.render(stack, value.getName(), windowX + 155.0F + x, windowY + valueHeight + motion + 19.0F + heightOffset, Color.WHITE, true, scale);
               x += currentLength;
            }
         }
         ...
         for (Value valuex : this.renderValues) {
            if (valuex.isVisible() && valuex.getValueType() == ValueType.FLOAT) {
               ...
               opensans.render(stack, valuex.getName(), windowX + 140.0F, windowY + valueHeight + motion + 25.0F, Color.WHITE, true, 0.4);
               String currentValue = Math.round(floatValue.getCurrentValue() * 100.0F) / 100.0F + " / " + floatValue.getMaxValue();
               opensans.render(
                  stack,
                  currentValue,
                  windowX + windowWidth - opensans.getWidth(currentValue, 0.4) - 15.0F,
         ...
         for (Value valuexx : this.renderValues) {
            if (valuexx.isVisible() && valuexx.getValueType() == ValueType.MODE) {
               ...
               animationx.update(true);
               opensans.render(stack, valuexx.getName(), windowX + 140.0F, windowY + valueHeight + motion + 25.0F, Color.WHITE, true, 0.4);
               x = 0.0F;
               valueHeight += 15.0F;
      ...
      if (this.draggingFloatValue != null) {
         float stage = (mouseX - windowX - 140.0F) / (windowWidth - 160.0F);
         ...
         valuexxx = Math.round(valuexxx / this.draggingFloatValue.getStep()) * this.draggingFloatValue.getStep();
         this.draggingFloatValue.setCurrentValue(valuexxx);
      }

      if (this.clickDragWindow && this.mouseDown) {
         windowX = windowX + (mouseX - this.dragMousePosition[0]);
         windowY = windowY + (mouseY - this.dragMousePosition[1]);
         this.SetDragPosition(mouseX, mouseY);
      }

      if (this.categoryModules != null && !this.hoveringBack && this.clickResizeWindow && this.mouseDown) {
         windowWidth = windowWidth + (mouseX - this.dragMousePosition[0]);
         windowHeight = windowHeight + (mouseY - this.dragMousePosition[1]);
      ...
      if (this.bindingAnimation.value > 6.0F) {
         ...
         opensans.render(
            stack,
            line1,
            windowX + this.widthAnimation.value / 2.0F - opensans.getWidth(line1, 0.6) / 2.0F,
            windowY + (this.heightAnimation.value - opensans.getHeight(true, 0.6)) / 2.0 - 10.0,
            Color.WHITE,
            true,
            0.6
         );
         opensans.render(
            stack,
            line2,
            windowX + this.widthAnimation.value / 2.0F - opensans.getWidth(line2, 0.4) / 2.0F,
            windowY + (this.heightAnimation.value - opensans.getHeight(true, 0.4)) / 2.0 + 15.0,
            Color.WHITE,
            true,
            0.4
         );
