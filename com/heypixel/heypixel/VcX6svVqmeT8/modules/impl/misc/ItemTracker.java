...
@ModuleInfo(
   name = "EntityTags",
   description = "Show the player's tags.",
   ...
   private void updatePositions(float renderPartialTicks) {
      ...
      for (Entity entity : mc.level.entitiesForRendering()) {
         if (entity != mc.player && entity instanceof AbstractClientPlayer) {
            ...
            double y = MathUtils.interpolate(renderPartialTicks, entity.yo, entity.getY()) + entity.getBbHeight();
      ...
      if (this.shared.getCurrentValue()) {
         ...
         for (SharedESPData value : dataMap.values()) {
            ...
            double y = value.getPosY() + mc.player.getBbHeight();
   ...
   public void onRender(EventRender2D e) {
      for (ItemTracker.TargetInfo info : this.entityPositions) {
         ...
         for (String entityTag : info.getDescription()) {
            Fonts.harmony
               .render(e.getStack(), I18n.get(entityTag, new Object[0]), info.getPosition().x + 10.0F, info.getPosition().y + y, Color.RED, true, 0.35F);
         ...
         if (this.debug.getCurrentValue() && info.getPlayer() != null) {
            ...
            for (String debugInfo : debugInfos) {
               Fonts.harmony.render(e.getStack(), debugInfo, info.getPosition().x + 10.0F, info.getPosition().y + y, Color.RED, true, 0.35F);
               y += Fonts.harmony.getHeight(true, 0.35F);
            }
         }
