...
import com.heypixel.heypixel.VcX6svVqmeT8.utils.MoveUtils;
...
public class InventoryCleaner extends Module {
   ...
   private List<ItemStack> getAllWeapons() {
      ...
      for (int i = 0; i < mc.player.getInventory().items.size(); i++) {
         ...
         if (!stack.isEmpty() && (stack.getItem() instanceof SwordItem || InventoryUtils.isSharpnessAxe(stack))) {
            weapons.add(stack);
         }
      }
   ...
   public boolean isItemUseful(ItemStack stack) {
      if (stack.isEmpty()) {
         ...
         return Critical.instance.overrideSwordSorting() || InventoryUtils.getBestSword() == stack;
         ...
         return Critical.instance.overrideSwordSorting() || InventoryUtils.getBestAxe() == stack;
   ...
   private boolean checkConfig() {
      ...
      if (Critical.instance.overrideSwordSorting() && usedSlot.contains(8)) {
         Notification notification = new Notification(NotificationLevel.ERROR, "You cannot use slot 9 when critical is enabled!", 10000L);
   ...
   public void onMotion(EventMotion e) {
      if (e.getType() == EventType.PRE) {
         ...
         if (this.switchBlock.getCurrentValue()) {
            ...
            if (InventoryUtils.getBlockCountInInventory() > this.maxBlockSize.getCurrentValue()) {
         ...
         if (this.switchSword.getCurrentValue()) {
            ...
            List<ItemStack> allWeapons = this.getAllWeapons();
            allWeapons.remove(bestSword);
            ItemStack bestStack = allWeapons.stream()
               .max(
                  Comparator.comparingInt(
                     s -> (int)(s.getItem() instanceof SwordItem ? InventoryUtils.getSwordDamage(s) * 100.0F : InventoryUtils.getAxeDamage(s) * 100.0F)
                  )
               )
               .orElse(null);
            if (Critical.instance.overrideSwordSorting() && bestStack != null) {
               int secondBestSlotIndex = 8;
               ItemStack currentSecondBest = (ItemStack)mc.player.getInventory().items.get(secondBestSlotIndex);
               float currentDamage = currentSecondBest.getItem() instanceof SwordItem
                  ? InventoryUtils.getSwordDamage(currentSecondBest)
                  : InventoryUtils.getAxeDamage(currentSecondBest);
               float bestWeaponDamage = bestStack.getItem() instanceof SwordItem
                  ? InventoryUtils.getSwordDamage(bestStack)
                  : InventoryUtils.getAxeDamage(bestStack);
               if (bestWeaponDamage > currentDamage) {
                  this.swapItem(secondBestSlotIndex, bestStack);
               }
            }
         }
         ...
         if (this.switchBow.getCurrentValue()) {
            ...
            if (InventoryUtils.getItemCount(Items.ARROW) > this.maxArrowSize.getCurrentValue()) {
         ...
         if (this.keepProjectile.getCurrentValue()) {
            if (InventoryUtils.getItemCount(Items.EGG) + InventoryUtils.getItemCount(Items.SNOWBALL) > this.maxProjectileSize.getCurrentValue()) {
