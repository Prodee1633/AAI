...
import com.heypixel.heypixel.VcX6svVqmeT8.events.impl.EventRender;
...
import com.heypixel.heypixel.VcX6svVqmeT8.utils.BlinkingPlayer;
...
import com.heypixel.heypixel.VcX6svVqmeT8.utils.InventoryUtils;
import com.heypixel.heypixel.VcX6svVqmeT8.utils.MathUtils;
import com.heypixel.heypixel.VcX6svVqmeT8.utils.ProjectionUtils;
import com.heypixel.heypixel.VcX6svVqmeT8.utils.RenderUtils;
import com.heypixel.heypixel.VcX6svVqmeT8.utils.RotationUtils;
import com.heypixel.heypixel.VcX6svVqmeT8.utils.SharedESPData;
import com.heypixel.heypixel.VcX6svVqmeT8.utils.StencilUtils;
import com.heypixel.heypixel.VcX6svVqmeT8.utils.Vector2f;
...
public class NameTags extends Module {
   ...
   public static boolean isAiming(Entity targetEntity, float yaw, float pitch) {
      Vec3 playerEye = new Vec3(mc.player.getX(), mc.player.getY() + mc.player.getEyeHeight(), mc.player.getZ());
      ...
      try {
         ...
         if (this.spawnPosition != null) {
            this.compassPosition = ProjectionUtils.project(
               this.spawnPosition.getX() + 0.5, this.spawnPosition.getY() + 1.75, this.spawnPosition.getZ() + 0.5, e.getRenderPartialTicks()
            );
         }
   ...
   public void onRender(EventRender2D e) {
      ...
      if (this.compassPosition != null) {
         ...
         float scale = Math.max(
               80.0F
                  - Mth.sqrt(
                     (float)mc.player
                        .distanceToSqr(this.spawnPosition.getX() + 0.5, this.spawnPosition.getY() + 1.75, this.spawnPosition.getZ() + 0.5)
                  ),
               0.0F
            )
            * this.scale.getCurrentValue()
            / 80.0F;
         String text = "Compass";
         float width = Fonts.harmony.getWidth(text, scale);
         double height = Fonts.harmony.getHeight(true, scale);
         this.blurMatrices
            .add(new Vector4f(position.x - width / 2.0F - 2.0F, position.y - 2.0F, position.x + width / 2.0F + 2.0F, (float)(position.y + height)));
         StencilUtils.write(false);
         RenderUtils.fill(e.getStack(), position.x - width / 2.0F - 2.0F, position.y - 2.0F, position.x + width / 2.0F + 2.0F, (float)(position.y + height), -1);
         StencilUtils.erase(true);
         RenderUtils.fill(
            e.getStack(), position.x - width / 2.0F - 2.0F, position.y - 2.0F, position.x + width / 2.0F + 2.0F, (float)(position.y + height), color1
         );
         StencilUtils.dispose();
         Fonts.harmony.setAlpha(0.8F);
         Fonts.harmony.render(e.getStack(), text, position.x - width / 2.0F, position.y - 1.0F, Color.WHITE, true, scale);
      }
      ...
      if (this.shared.getCurrentValue()) {
         for (NameTags.NameTagData data : this.sharedPositions) {
            ...
            float scale = this.scale.getCurrentValue();
            float width = Fonts.harmony.getWidth(textx, scale);
            double delta = 1.0 - data.getHealth() / data.getMaxHealth();
            double height = Fonts.harmony.getHeight(true, scale);
            this.blurMatrices
               .add(new Vector4f(positionx.x - width / 2.0F - 2.0F, positionx.y - 2.0F, positionx.x + width / 2.0F + 2.0F, (float)(positionx.y + height)));
            RenderUtils.fill(
               e.getStack(), positionx.x - width / 2.0F - 2.0F, positionx.y - 2.0F, positionx.x + width / 2.0F + 2.0F, (float)(positionx.y + height), color1
            );
            RenderUtils.fill(
               e.getStack(),
               positionx.x - width / 2.0F - 2.0F,
               positionx.y - 2.0F,
               (float)(positionx.x + width / 2.0F + 2.0F - (width + 4.0F) * delta),
               (float)(positionx.y + height),
               color2
            );
            Fonts.harmony.setAlpha(0.8F);
            Fonts.harmony.render(e.getStack(), textx, positionx.x - width / 2.0F, positionx.y - 1.0F, Color.WHITE, true, scale);
            Fonts.harmony.setAlpha(1.0F);
            e.getStack().popPose();
         }
      }
   ...
   private void updatePositions(float renderPartialTicks) {
      ...
      for (Entity entity : mc.level.entitiesForRendering()) {
         if (entity instanceof Player && !entity.getName().getString().startsWith("CIT-")) {
            ...
            double y = MathUtils.interpolate(renderPartialTicks, entity.yo, entity.getY()) + entity.getBbHeight() + 0.5;
      ...
      if (this.shared.getCurrentValue()) {
         ...
         for (SharedESPData value : dataMap.values()) {
            ...
            double y = value.getPosY() + mc.player.getBbHeight() + 0.5;
